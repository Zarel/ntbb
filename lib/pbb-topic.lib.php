...
class PBB_Topic {
	function getTopic($topic_id, $view, $page = 1, $forum_id = 0) {
		global $db, $auth, $user;
		...
		$sort_days	= (!empty($user->data['user_post_show_days'])) ? $user->data['user_post_show_days'] : 0;
		$sort_key	= (!empty($user->data['user_post_sortby_type'])) ? $user->data['user_post_sortby_type'] : 't';
		$sort_dir	= (!empty($user->data['user_post_sortby_dir'])) ? $user->data['user_post_sortby_dir'] : 'a';
		...
		$retval = $topic_data + array(
			...
			'page' => intval($start/$config['posts_per_page'])+1,
			'page_count' => intval(($total_posts-1)/$config['posts_per_page'])+1,
			'can_reply' => ($auth->acl_get('f_reply', $forum_id)),
		);
		...
		for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
		{
			...
			$retval['posts'][] = $row + array(
				'author' => ($poster_id != ANONYMOUS) ? $user_cache[$poster_id]['author_username'] : get_username_string('username', $poster_id, $row['username'], $row['user_colour'], $row['post_username']),
				'author_id' => $poster_id,
				'author_avatar' => $user_cache[$poster_id]['avatar'],
				'treeindex' => '',
				'message_html' => $message,
				'signature_html' => ($row['enable_sig']) ? $user_cache[$poster_id]['sig'] : '',
				'number' => $i + $start + 1,
